...
async function main() {
  ...
  const indianStates = [
    'Andhra Pradesh', 'Karnataka', 'Tamil Nadu', 'Telangana', 'Kerala',
        ...
        data: {
          ...
          fields: {
            create: Array.from({ length: faker.number.int({ min: 1, max: 3 }) }, () => ({
              areaHa: parseFloat(faker.number.float({ min: 0.5, max: 10, precision: 0.01 }).toFixed(2)),
              yieldEstimate: parseFloat(faker.number.float({ min: 2, max: 15, precision: 0.1 }).toFixed(1)),
