				...
				{fields.map((_: any, index: number) => (
					<Card key={index}>
						<CardHeader className="flex flex-row items-center justify-between pb-2">
							...
							{fields.length > 1 && (
								<Button type="button" onClick={() => removeField(index)} variant="ghost" size="sm" className="h-8 w-8 p-0">
						...
						<CardContent>
							<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormControl>
												<Input type="number" placeholder="Enter area" {...field} onChange={(e) => field.onChange(parseFloat(e.target.value) || 0)} value={field.value || ''} />
											</FormControl>
											<FormMessage />
										</FormItem>
									)}
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormControl>
												<Input type="number" placeholder="Enter yield estimate" {...field} onChange={(e) => field.onChange(parseFloat(e.target.value) || 0)} value={field.value || ''} />
											</FormControl>
											<FormMessage />
										</FormItem>
									)}
								...
								<div className="sm:col-span-2 space-y-4">
									...
									<LocationButton onLocationUpdate={(locationData: any) => handleLocationUpdate(index, locationData)} fieldIndex={index} />
										...
										<FormField
											...
											render={({ field }) => (
												<FormItem>
													...
													<FormControl>
														<Input type="number" placeholder="Enter latitude" {...field} onChange={(e) => field.onChange(parseFloat(e.target.value) || 0)} value={field.value || ''} />
													</FormControl>
													<FormMessage />
												</FormItem>
											)}
										...
										<FormField
											...
											render={({ field }) => (
												<FormItem>
													...
													<FormControl>
														<Input type="number" placeholder="Enter longitude" {...field} onChange={(e) => field.onChange(parseFloat(e.target.value) || 0)} value={field.value || ''} />
													</FormControl>
													<FormMessage />
												</FormItem>
											)}
