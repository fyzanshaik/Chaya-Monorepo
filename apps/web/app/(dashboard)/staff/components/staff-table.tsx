// app/staff/components/staff-table.tsx
'use client';
...
export function StaffTable() {
	...
	useEffect(() => {
		if (searchQuery.trim() === '') {
			...
			setFilteredUsers(users.filter((user) => user.name.toLowerCase().includes(query) || user.email.toLowerCase().includes(query) || user.role.toLowerCase().includes(query)));
		}
	}, [searchQuery, users]);
	...
	const fetchUsers = async () => {
		...
		try {
			...
			console.log('Fetching users:', response.data.users);
		...
		<div className="space-y-4">
			<div className="p-4 border-b flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				<div className="relative w-full sm:w-64">
					...
					<Input placeholder="Search staff members..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
				</div>
								...
								<TableRow key={user.id} className="hover:bg-gray-50">
									...
									<TableCell>
										<div className="flex items-center space-x-2">
											<Switch checked={user.isEnabled} disabled={user.role === 'ADMIN'} onCheckedChange={() => toggleUserStatus(user)} />
											<span className={user.isEnabled ? 'text-green-600' : 'text-red-600'}>{user.isEnabled ? 'Enabled' : 'Disabled'}</span>
										</div>
									</TableCell>
									<TableCell>
									...
									<TableCell className="text-right">
										<DropdownMenu>
											...
											<DropdownMenuContent align="end">
												...
												<DropdownMenuItem onClick={() => handleDeleteUser(user)} disabled={user.role === 'ADMIN'} className={user.role === 'ADMIN' ? 'opacity-50' : 'text-red-600'}>
			...
			{/* Edit Dialog */}
			{editingUser && <EditStaffDialog user={editingUser} open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen} onUserUpdated={fetchUsers} />}
			...
			<AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This action will permanently delete the staff member account for <span className="font-semibold">{userToDelete?.name}</span>. This action cannot be undone.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
